...
async def update_my_company(data: CompanyUpdate, user=Depends(require_admin)):
...
async def create_property(data: PropertyCreate, user=Depends(require_admin)):
    ...
    # Auto-link assigned team members to this property and generate tasks
    if prop.get("assigned_cohost"):
        await link_staff_to_property(company_id, prop["assigned_cohost"], prop["id"])
    if prop.get("assigned_housekeeper"):
        await link_staff_to_property(company_id, prop["assigned_housekeeper"], prop["id"])
...
async def auto_generate_tasks_for_staff(company_id: str, staff: dict, property_ids: list = None):
    """
    Auto-generate tasks for a staff member across their assigned properties.
    Called when staff is created or when new properties are assigned.
    ...
    # Use provided property_ids or all assigned properties
    target_properties = property_ids or staff.get("assigned_properties", [])
    if not target_properties:
        return
    
    today = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    
    # Find all future bookings on the target properties
    bookings = await db.bookings.find({
        "company_id": company_id,
        "property_id": {"$in": target_properties},
        "booking_type": {"$ne": "blocked"},
        "check_out": {"$gte": today}
    }, {"_id": 0}).to_list(1000)
    ...
    for booking in bookings:
        ...
        # Check if task already exists for this staff+booking combination
        ...
        elif staff_role in ["co_host", "cohost"]:
            # Check-in task
            tasks_to_create.append({
                "id": f"task_{uuid.uuid4().hex[:12]}",
                "company_id": company_id,
...
async def cancel_invitation(inv_id: str, user=Depends(require_admin)):
    ...
    # Generate unique cancelled token to avoid duplicate key error on unique index
    # Using a prefix so it can be easily identified as cancelled
    cancelled_token = f"cancelled_{uuid.uuid4().hex}"
    
    await db.invitations.update_one(
        {"id": inv_id},
        {"$set": {"status": "cancelled", "token": cancelled_token, "updated_at": datetime.now(timezone.utc).isoformat()}}
    )
...
async def startup():
    ...
    await db.payouts.create_index([("company_id", 1)])
